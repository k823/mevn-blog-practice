<template>
  <!-- When user is author -->
  <span v-if="canModify">
    <router-link class="btn btn-sm btn-outline-warning" :to="editArticleLink">
      <i class="ion-edit"></i> <span>&nbsp;Editar Artículo</span>
    </router-link>
    <span>&nbsp;&nbsp;</span>
    <button class="btn btn-outline-danger btn-sm" @click="deleteArticle">
      <i class="ion-trash-a"></i> <span>&nbsp;Borrar Artículo</span>
    </button>
  </span>
  <!-- Used in ArticleView when not author -->
  <span v-else>
    <button class="btn btn-sm btn-outline-warning" @click="toggleFollow">
...
export default {
  ...
  computed: {
    ...
    followUserLabel() {
      return `${this.profile.following ? "Dejar de seguir" : "Seguir"} ${
        this.article.author.username
      }`;
    },
    favoriteArticleLabel() {
      return this.article.favorited ? "Ya no me interesa" : "Me encanta!";
    },
    favoriteCounter() {
